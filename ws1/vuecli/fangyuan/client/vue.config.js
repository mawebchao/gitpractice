    ...
    devServer: {
        ...
        proxy: {
            ...
            '/idea': {   //  拦截以 /api 开头的接口
                ...
                secure: false, // 如果是https接口，需要配置这个参数
                pathRewrite: {
                    '^/idea': '/'  //这里理解成用‘/api’代替target里面的地址，后面组件中我们掉接口时直接用api代替 比如我要调用'http://40.00.100.100:3002/api/user/add'，直接写‘/api/user/add’即可
                }
            },
            '/koa': {   //  拦截以 /api 开头的接口
                ...
                secure: false, // 如果是https接口，需要配置这个参数
                pathRewrite: {
                    '^/koa': '/'  //这里理解成用‘/api’代替target里面的地址，后面组件中我们掉接口时直接用api代替 比如我要调用'http://40.00.100.100:3002/api/user/add'，直接写‘/api/user/add’即可
                }
            }
        }
    }
