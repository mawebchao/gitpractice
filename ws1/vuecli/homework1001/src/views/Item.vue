  ...
  <div>
    ...
    <el-card class="box-card">
      ...
      <el-table :data="itemList" style="width: 100%" stripe border>
        ...
        <el-table-column prop="title" label="商品标题" width="400px"></el-table-column>
        <el-table-column prop="sellPoint" label="卖点信息" width="300px"></el-table-column>
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button
              ...
              icon="el-icon-edit"
              size="mini"
              @click="updateItemBtn(scope.row)"
            >修改</el-button>
            <el-button
              type="danger"
              icon="el-icon-delete"
              size="mini"
              @click="deleteItemBtn(scope.row)"
            >删除</el-button>
          </template>
        </el-table-column>
      </el-table>
...
export default class extends Vue {
  ...
  //实现商品信息分页查询
  async getItemList() {
    const { data: result } = await this.$axios.get("/item/getItemList", {
  ...
  async updateStatus(item) {
    const { data: result } = await this.$axios.put("/item/updateItemStatus", {
      id: item.id,
      status: item.status
    });
