...
export default class extends Vue {
  ...
  closeDialog() {
    //重置表格数据
    this.$addUserRef.resetFields();
  }
  //校验用户数据
  addUserBtn() {
    // console.log(ref.validate);
    this.$addUserRef =this.$refs.addUserRef;
    (this.$addUserRef as Form).validate(async valid => {
      //如果校验失败 则停止数据
      if (!valid) return;
  ...
  async updateUserBtn(user) {
    ...
    // console.log(user)
    // const { data: result } = await this.$axios.get("/user/" + user.id);
  ...
  closeUpdateDialog() {
    ...
    (this.$updateUserRef as Form).resetFields();
  }
  updateUser() {
    //1.预校验数据
  ...
  rules = {
    ...
    password2: [
      ...
      //     validator: () => {
      //       // console.log(this)
      //       checkPassword.bind(this.addUserModel)();
      //     },
      //     trigger: "blur"
      //   }
