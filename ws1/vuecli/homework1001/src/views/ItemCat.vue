<template>
  <div>
    ...
    <el-dialog
      title="新增商品分类"
      :visible.sync="addItemCatDialogVisible"
      width="50%"
      @close="closeAddItemCatDialog"
    >
      <!-- 定义分类表单 -->
      <el-form :model="itemCatForm" :rules="rules" ref="itemCatFormRef" label-width="100px">
        ...
        <el-form-item label="父级分类名称:">
          ...
          <el-cascader
            ...
            :props="props"
            :options="parentItemCatList"
            clearable
            @change="parentItemCatChange"
          ></el-cascader>
        </el-form-item>
      </el-form>
    ...
    <el-dialog title="修改商品分类" :visible.sync="updateItemCatDialogVisible" width="50%">
      <!-- 定义分类表单 -->
      <el-form
        :model="updateItemCatForm"
        :rules="rules"
        ref="upDateItemCatForm"
        label-width="100px"
      >
        <el-form-item label="分类名称:" prop="name">
...
export default class extends Vue {
  ...
  batchDelete() {
    this.$confirm("此操作将永久删除该数据, 是否继续?", "提示", {
      ...
      .then(async () => {
        let ids:Array<number> = this.multipleSelection.map((item: any) => item.id);
        // console.log(this.multipleSelection,JSON.stringify(ids))
        // //传递分类id
        const { data: result } = await this.$axios.post(
          "/itemCat/batchDelete",
          {
            // params: { delitemsStr: JSON.stringify(ids).replace('[','').replace(']','') }
            idList: JSON.stringify(ids)
          }
        );
        if (result.status !== 200)
          return this.$message.error("删除商品分类失败");
        this.$message.success("删除数据成功,删除了" + result.data + "条数据");
        //删除成功之后,刷新页面数据
  ...
  parentItemCatChange() {
    ...
    if (this.selectedKeys.length > 0) {
      //[1,2] 主要获取最后一位
      this.itemCatForm.parentId = this.selectedKeys[
  ...
  async addItemCatForm() {
    //先将整个表单进行校验
    this.$itemCatFormRef = this.$refs.itemCatFormRef;
    (this.$itemCatFormRef as Form).validate(async validate => {
      if (!validate) return;
      const { data: result } = await this.$axios.post(
        "/itemCat/saveItemCat",
        this.itemCatForm
      );
      console.log("新增商品分类结果：", result);
  ...
  initItemCatForm() {
    this.$itemCatFormRef.resetFields();
    //清空form提交其他数据
    this.itemCatForm.parentId = 0;
    this.itemCatForm.level = 1;
    //清空级联选择框的数组
  ...
  updateItemCatBtn(itemCat) {
    console.log("修改选中的当前行信息为：", itemCat);
  ...
  async updateItemCat() {
    //修改商品分类信息
    const { data: result } = await this.$axios.put(
      "/itemCat/updateItemCat",
      this.updateItemCatForm
    );
    console.log("更新itemcat结果为：", result);
    ...
    this.$confirm("此操作将永久删除该数据, 是否继续?", "提示", {
      ...
      .then(async () => {
        ...
        const { data: result } = await this.$axios.delete(
          "/itemCat/deleteItemCat",
          {
            params: { id: itemCat.id, level: itemCat.level }
          }
        );
        console.log("删除商品分类结果：", result);
        if (result.status !== 200)
          return this.$message.error("删除商品分类失败");
        this.$message.success("删除数据成功,删除了" + result.data + "条数据");
        //删除成功之后,刷新页面数据
